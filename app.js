...
function renderGenericConversation() {
  const conversation = state.activeConversation || { title: "BrainT conversation" };
  const sourceDescription = state.sourceScope === "both"
    ? "Workspace and trusted external sources"
    : state.sourceScope === "workspace"
      ? "MRANTI Workspace sources"
      : "Trusted external sources";
  if (state.loading) setOrbState("thinking");
  const messages = state.conversationMessages.map(renderMessage).join("");
  elements.viewRoot.innerHTML = `<section class="conversation-view" aria-labelledby="conversation-title"><div class="conversation-container">
    <header class="conversation-topline"><div><div class="conversation-kicker">MRANTI BrainT Workspace</div><h1 id="conversation-title" class="conversation-title">${escapeHtml(conversation.title || "BrainT conversation")}</h1><p class="conversation-meta">${escapeHtml(sourceDescription)} · secured by your authorised account</p></div></header>
    <div class="response-stage">
      <aside class="response-presence" aria-label="BrainT status">${renderOrb("response-orb")}<span class="response-state-copy" data-orb-state-copy></span></aside>
      <div class="chat-thread">${messages}${state.loading ? `<article class="assistant-message loading-message" aria-live="polite"><span class="loading-pulse" aria-hidden="true"></span><div class="loading-copy"><strong>BrainT is working</strong><p>Reviewing authorised sources and preparing a grounded response.</p></div></article>` : ""}</div>
    </div>
    <div class="persistent-composer-wrap">${renderComposer({ id: "generic-composer", placeholder: "Ask a follow-up or request more detail…" })}</div>
  </div></section>`;
...
function renderMessage(message) {
  if (message.role === "user") {
    return `<article class="request-card"><span class="request-icon">${icon("user", 18)}</span><div>${formatAnswer(message.text)}</div>${message.attachments?.length ? message.attachments.map(renderAttachmentPreview).join("") : ""}</article>`;
  }
  if (message.role === "error") {
    return `<article class="assistant-message error-message"><div class="assistant-heading"><strong>BrainT needs attention</strong><small>Request not completed</small></div><div class="assistant-answer">${formatAnswer(message.text)}</div></article>`;
  }
  return `<article class="assistant-message"><div class="assistant-heading"><strong>BrainT response</strong><small>Just now</small></div><div class="assistant-answer">${formatAnswer(message.text)}</div>${renderStructuredArtifact(message.artifact)}${renderAnswerSources(message)}</article>`;
}

...
function renderPresentationIntelligenceWorkspace(artifact, message, sources) {
  ...
  const headlineStats = [
    ["Slides analysed", slideCount || "—"],
    ["Decision asks", decisions.length],
    ["Attention items", risks.length],
    ["People to contact", contributors.length]
  ];
  const topSources = Array.isArray(sources) ? sources.slice(0, 4) : [];

  return `<section class="presentation-intelligence presentation-command-center" aria-label="Presentation intelligence workspace">
    <div class="presentation-command-grid">
      <section class="presentation-executive-snapshot presentation-command-hero">
        <div class="presentation-panel-heading presentation-hero-heading">
          <div><span class="intel-panel-kicker">Executive command view</span><h2>${escapeHtml(presentation.name || artifact.title || "Presentation intelligence")}</h2>${latestUpdate ? `<p class="presentation-hero-meta">Latest material update · ${escapeHtml(latestUpdate)}</p>` : ""}</div>
          <span class="intel-live-badge">${icon("check", 12)} Deck read</span>
        </div>
        <div class="presentation-status-grid">${headlineStats.map(([label,value]) => `<div class="presentation-status-card"><span>${escapeHtml(label)}</span><strong>${escapeHtml(value)}</strong></div>`).join("")}</div>
        <div class="presentation-executive-copy">
          <span class="presentation-assessment-label">BrainT assessment</span>
          <strong>${escapeHtml(artifact.overallAssessment || "Current assessment")}</strong>
      ...
      <section class="presentation-panel presentation-decision-panel">
        <div class="presentation-panel-heading"><div><span class="intel-panel-kicker">Decision intelligence</span><h2>CEO decision queue</h2></div><span class="presentation-panel-count attention">${decisions.length}</span></div>
        ${renderPresentationIntelList(decisions,{limit:3,numbered:true})}
        ${renderPresentationPanelToggle(decisions.length,3,"decisions")}
      </section>

      <section class="presentation-panel presentation-material-panel">
        <div class="presentation-panel-heading"><div><span class="intel-panel-kicker">Latest material</span><h2>Source deck</h2></div></div>
        <div class="presentation-material-card presentation-material-card-stacked">${thumbnail ? `<img src="${escapeHtml(thumbnail)}" alt="First slide preview of ${escapeHtml(presentation.name || "presentation")}" referrerpolicy="no-referrer"/>` : `<div class="presentation-material-fallback">${icon("folder", 28)}</div>`}<div class="presentation-material-copy"><strong>${escapeHtml(presentation.name || "Google Slides presentation")}</strong><p>${escapeHtml([slideCount ? `${slideCount} slides` : "", presentation.owner, presentation.location].filter(Boolean).join(" · "))}</p><div class="presentation-material-actions">${presentation.url ? `<button type="button" class="drive-action-primary" data-source-url="${escapeHtml(presentation.url)}">${icon("external", 14)} Open deck</button>` : ""}</div></div></div>
      </section>

      <section class="presentation-panel contacts-panel">
        <div class="presentation-panel-heading"><div><span class="intel-panel-kicker">People & contributors</span><h2>Who to contact</h2></div><span class="presentation-panel-count">${contributors.length}</span></div>
        ${renderContributorCards(contributors,3)}
        ${renderPresentationPanelToggle(contributors.length,3,"contacts")}
      </section>

      <section class="presentation-panel risk-panel">
        <div class="presentation-panel-heading"><div><span class="intel-panel-kicker">Attention</span><h2>Risks & gaps</h2></div><span class="presentation-panel-count risk">${risks.length}</span></div>
        ${renderPresentationIntelList(risks,{limit:3})}
        ${renderPresentationPanelToggle(risks.length,3,"risks")}
      </section>

      <section class="presentation-panel presentation-strategy-panel">
        <div class="presentation-panel-heading"><div><span class="intel-panel-kicker">Strategy</span><h2>Priorities & key themes</h2></div></div>
        <div class="presentation-strategy-split">
          <div class="presentation-subpanel"><div class="presentation-subpanel-title"><span>${icon("target", 14)}</span><strong>Strategic priorities</strong></div>${renderPresentationIntelList(priorities,{limit:3})}${renderPresentationPanelToggle(priorities.length,3,"priorities")}</div>
          <div class="presentation-subpanel"><div class="presentation-subpanel-title"><span>${icon("sparkles", 14)}</span><strong>Key themes</strong></div>${renderPresentationIntelList(themes,{limit:3})}${renderPresentationPanelToggle(themes.length,3,"themes")}</div>
        </div>
      </section>

      <section class="presentation-panel presentation-metrics-panel">
        <div class="presentation-panel-heading"><div><span class="intel-panel-kicker">Evidence in deck</span><h2>Metrics & claims</h2></div><span class="presentation-panel-count">${metrics.length}</span></div>
        ${renderPresentationIntelList(metrics,{metric:true,detailKey:"context",limit:4})}
        ${renderPresentationPanelToggle(metrics.length,4,"findings")}
      </section>

      <section class="presentation-panel actions-panel">
        <div class="presentation-panel-heading"><div><span class="intel-panel-kicker">BrainT recommendation</span><h2>Recommended next actions</h2></div></div>
      ...
      <section class="presentation-panel presentation-evidence-panel" id="intelligence-sources">
        <div class="presentation-panel-heading"><div><span class="intel-panel-kicker">Evidence quality</span><h2>Grounding & sources</h2></div><span class="intel-live-badge subtle">${icon("shield", 12)} Workspace</span></div>
        <p class="presentation-evidence-note">${escapeHtml(artifact.evidenceNote || "Contacts and findings are derived only from verified Workspace evidence available to BrainT.")}</p>
        <div class="presentation-evidence-stats"><span>${slideCount} slides</span><span>${Number(stats.commentsReviewed || 0)} comments</span><span>${Number(stats.peopleSignals || 0)} people signals</span></div>
        ${topSources.length ? `<div class="presentation-source-list">${topSources.map(source => `<button type="button" class="presentation-source-row" data-source-url="${escapeHtml(source.url || source.webViewLink || "")}" ${source.url || source.webViewLink ? "" : "disabled"}><span class="presentation-source-type">${escapeHtml(sourceTypeLabel(source))}</span><span class="presentation-source-copy"><strong>${escapeHtml(source.title || source.name || "Source")}</strong>${source.meta ? `<small>${escapeHtml(source.meta)}</small>` : ""}</span>${source.url || source.webViewLink ? icon("external", 12) : ""}</button>`).join("")}</div>` : ""}
      </section>
    </div>
  </section>`;
}
...
function renderGenericConversation(options = {}) {
  ...
  elements.viewRoot.innerHTML = `<section class="conversation-view intelligence-view" aria-labelledby="intelligence-title"><div class="intelligence-container" id="intelligence-top">
    ...
    ${renderConversationContext(turn)}
    ${turn.user ? `<div class="intelligence-current-request" id="current-intelligence-request"><span class="request-icon">${icon("user", 17)}</span><div><span class="request-label">Current request</span><p>${escapeHtml(turn.user.text)}</p></div></div>` : ""}
  ...
  elements.viewRoot.querySelectorAll("[data-presentation-toggle]").forEach(button => button.addEventListener("click", () => {
    const panel = button.closest(".presentation-panel, .presentation-subpanel");
    if (!panel) return;
  ...
  elements.viewRoot.querySelectorAll("[data-presentation-summary-toggle]").forEach(button => button.addEventListener("click", () => {
    const summary = button.closest(".presentation-executive-copy");
    if (!summary) return;

...
function renderComposer({ id, placeholder, home = false }) {
  ...
  const scopeLabel = state.sourceScope === "workspace"
    ...
    : state.sourceScope === "external"
      ...
      : "Workspace and trusted external sources";
  return `
    <form id="${id}" class="chat-composer${home ? " home-composer" : " persistent-composer"}" novalidate>
      <div class="composer-shell">
        ...
        <select class="source-dropdown" aria-label="Source scope" title="Choose sources">
          <option value="both" ${state.sourceScope === "both" ? "selected" : ""}>Workspace + web</option>
          <option value="workspace" ${state.sourceScope === "workspace" ? "selected" : ""}>Workspace only</option>
          <option value="external" ${state.sourceScope === "external" ? "selected" : ""}>External only</option>
        </select>
...
function renderGenericConversation(options = {}) {
  ...
  const sourceDescription = state.sourceScope === "both"
    ? "Workspace + trusted external"
    : state.sourceScope === "workspace"
      ? "MRANTI Workspace"
      : "Trusted external";
  ...
  const historyCount = Math.max(0, (state.conversationMessages.filter(message => message.role === "user").length || 1) - 1);
  ...
  elements.viewRoot.innerHTML = `<section class="conversation-view intelligence-view" aria-labelledby="intelligence-title"><div class="intelligence-container" id="intelligence-top">
    ...
    ${turn.user ? `<div class="intelligence-current-request"><span class="request-icon">${icon("user", 17)}</span><div><span class="request-label">Current request</span><p>${escapeHtml(turn.user.text)}</p></div>${historyCount ? `<span class="request-history">${historyCount} earlier turn${historyCount === 1 ? "" : "s"} retained</span>` : ""}</div>` : ""}
  ...
  requestAnimationFrame(() => {
    ...
    if (!scroller) return;
    if (options.focusCurrentTurn) scroller.scrollTo({ top: 0, behavior: "smooth" });

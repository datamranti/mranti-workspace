...
const elements = {
  ...
  workspaceSelector: document.querySelector("#workspace-selector"),
...
async function signOut() {
  ...
  document.body.classList.remove("authenticated", "sidebar-open", "inspector-open");
  showLoginError("You have signed out securely.", false);
}
...
function initialiseAuth() {
  ...
  firebaseAuth.onAuthStateChanged(async user => {
    ...
    if (state.googleAccessToken) showWorkspace(user);
      ...
      document.body.classList.remove("authenticated", "auth-pending");
      showLoginError("Continue with Google to authorise the Workspace services used by AI Brain.", false);
    }
  });
}
...
function initialiseApplicationShell() {
  document.querySelector("#mobile-menu-icon").innerHTML = icon("menu", 21);
  document.querySelector("#plus-icon").innerHTML = icon("plus", 18);
  document.querySelector("#workspace-selector-icon").innerHTML = icon("building", 17);
  document.querySelector("#workspace-chevron").innerHTML = icon("chevronDown", 14);
  document.querySelector("#profile-chevron").innerHTML = icon("chevronDown", 13);
  document.querySelector("#notification-icon").innerHTML = icon("bell", 20);
...
function renderRecentConversations() {
  ...
  if (!state.recentConversations.length) {
    elements.recentConversations.innerHTML = `<p class="recent-empty">No recent conversations</p>`;
    return;
  }
  elements.recentConversations.innerHTML = state.recentConversations.map(item => `
    <button class="recent-item${state.route.name === "conversation" && state.route.id === item.id ? " active" : ""}" type="button" data-conversation-id="${escapeHtml(item.id)}">
      <span>${icon("message", 15)}</span><span class="sidebar-label">${escapeHtml(item.title || "AI Brain conversation")}</span>
    </button>`).join("");
...
function closePopovers() {
  [elements.workspaceMenu, elements.notificationMenu, elements.profileMenu].forEach(menu => menu?.classList.add("hidden"));
  [elements.workspaceSelector, elements.notificationButton, elements.profileButton].forEach(button => button?.setAttribute("aria-expanded", "false"));
}
...
function parseRoute() {
  ...
  if (["saved", "projects"].includes(parts[0])) return { name: parts[0] };
  return { name: "home" };
}
...
function setActiveNavigation() {
  document.querySelectorAll("[data-route]").forEach(button => {
    button.classList.toggle("active", (state.route.name === "home" && button.dataset.route === "home") || button.dataset.route === state.route.name);
  });
  renderRecentConversations();
}
...
async function runLiveConversation(message) {
  ...
  try {
    ...
    await saveConversation(uid(), {
      id: conversationId,
      title: response.title || state.activeConversation.title,
      type: "query",
      status: "complete",
...
function renderMessage(message) {
  ...
  const sources = [...(message.internalSources || []), ...(message.citations || [])];
  return `<article class="assistant-message"><img src="mranti-workspace-mark.png" alt=""/><div><div class="assistant-answer">${formatAnswer(message.text)}</div>${sources.length ? `<div class="answer-sources"><strong>Sources</strong>${sources.map(source => `<button type="button" data-source-url="${escapeHtml(source.url || source.webViewLink || "")}">${escapeHtml(source.title || source.name || "Source")} ${icon("external", 10)}</button>`).join("")}</div>` : ""}</div></article>`;
}
...
function handleServiceError(error) {
  ...
  if (error?.authError) {
    ...
    showLoginError(error.message || "Your Workspace authorisation expired. Continue with Google to reconnect.", true);
    return;
  }
...
elements.workspaceSelector.addEventListener("click", event => { event.stopPropagation(); togglePopover(elements.workspaceSelector, elements.workspaceMenu); });
elements.notificationButton.addEventListener("click", event => { event.stopPropagation(); togglePopover(elements.notificationButton, elements.notificationMenu); });
...
elements.mobileNavButton.addEventListener("click", () => { const open = !document.body.classList.contains("sidebar-open"); document.body.classList.toggle("sidebar-open", open); elements.mobileNavButton.setAttribute("aria-expanded", String(open)); });
...
document.addEventListener("click", event => {
  if (![elements.workspaceMenu, elements.notificationMenu, elements.profileMenu, elements.workspaceSelector, elements.notificationButton, elements.profileButton].some(node => node?.contains(event.target))) closePopovers();
});

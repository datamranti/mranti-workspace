...
function renderDriveSearchWorkspace(artifact, message, sources) {
  const matches = Array.isArray(artifact?.matches) ? artifact.matches : [];
  if (!matches.length) {
    return `<section class="intel-panel drive-search-empty"><div class="intel-panel-head"><div><span class="intel-panel-kicker">Google Drive</span><h2>No verified match returned</h2></div></div><div class="intel-narrative">${formatAnswer(message.text)}</div></section>${renderEvidencePanel(sources)}`;
  }
  const [top, ...rest] = matches;
  const topMeta = [driveFileTypeLabel(top.mimeType), top.modifiedLabel, top.location, top.owner].filter(Boolean).join(" · ");
  return `<section class="drive-intelligence" aria-label="Google Drive intelligence">
    <div class="drive-hero-card">
      <div class="drive-hero-icon">${icon("folder", 28)}</div>
      <div class="drive-hero-copy"><span class="intel-panel-kicker">Best Workspace match</span><h2>${escapeHtml(top.name || "Workspace file")}</h2><p>${escapeHtml(topMeta)}</p>${top.description ? `<small>${escapeHtml(top.description)}</small>` : ""}</div>
      ${top.url ? `<button class="drive-open-primary" type="button" data-source-url="${escapeHtml(top.url)}">Open ${icon("external", 13)}</button>` : ""}
    </div>
    ${rest.length ? `<div class="drive-result-grid">${rest.map(file => `<button class="drive-result-card" type="button" data-source-url="${escapeHtml(file.url || "")}" ${file.url ? "" : "disabled"}><span class="drive-result-icon">${icon("folder", 17)}</span><span><strong>${escapeHtml(file.name || "Workspace file")}</strong><small>${escapeHtml([driveFileTypeLabel(file.mimeType), file.modifiedLabel, file.location].filter(Boolean).join(" · "))}</small></span>${file.url ? icon("external", 12) : ""}</button>`).join("")}</div>` : ""}
    <div class="drive-search-summary"><span class="intel-panel-kicker">BrainT synthesis</span><div class="intel-narrative">${formatAnswer(message.text)}</div></div>
  </section>${renderEvidencePanel(sources)}`;
}
  ...
  elements.viewRoot.innerHTML = `<section class="conversation-view intelligence-view" aria-labelledby="intelligence-title"><div class="intelligence-container" id="intelligence-top">
    <header class="intelligence-header">
      <div class="intelligence-heading-copy">
        ...
        <p class="intelligence-subtitle">Latest Intelligence Brief</p>
      ...
      <div class="intelligence-toolbar">
        ${sources.length ? `<button class="intel-toolbar-button" type="button" data-scroll-sources>${icon("folder", 15)} View all sources</button>` : ""}

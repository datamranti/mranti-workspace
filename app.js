...
function renderPresentationIntelList(items, { titleKey = "title", detailKey = "detail", metric = false } = {}) {
  ...
  return `<div class="presentation-intel-list">${items.map(item => `<article class="presentation-intel-item">
...
function renderContributorCards(contributors) {
  ...
  return `<div class="contributor-list">${contributors.slice(0, 5).map(person => `<article class="contributor-card">
    ...
    <div class="contributor-copy"><div class="contributor-name-row"><strong>${escapeHtml(person.name || person.email || "Workspace contributor")}</strong><span class="contributor-confidence ${escapeHtml(person.confidence || "evidence-based")}">${escapeHtml(person.confidence === "verified" ? "Verified" : "Evidence-based")}</span></div>
      ...
      <p>${escapeHtml(person.whyContact || person.evidenceDetail || "")}</p>
...
function renderPresentationIntelligenceWorkspace(artifact, message, sources) {
  ...
  const statusCards = [
    ["Slides analysed", stats.slidesAnalysed || presentation.slideCount || ""],
    ["Comments reviewed", stats.commentsReviewed ?? ""],
    ["People signals", stats.peopleSignals ?? ""],
    ["Latest material update", presentation.modifiedTime ? formatDateTime(presentation.modifiedTime) : ""]
  ].filter(([, value]) => value !== "" && value !== null && value !== undefined);
  return `<section class="presentation-intelligence" aria-label="Presentation intelligence workspace">
    <section class="presentation-executive-snapshot">
      <div class="presentation-panel-heading"><div><span class="intel-panel-kicker">Executive snapshot</span><h2>${escapeHtml(presentation.name || artifact.title || "Presentation intelligence")}</h2></div><span class="intel-live-badge">${icon("check", 12)} Deck read</span></div>
      ${statusCards.length ? `<div class="presentation-status-grid">${statusCards.map(([label,value]) => `<div class="presentation-status-card"><span>${escapeHtml(label)}</span><strong>${escapeHtml(value)}</strong></div>`).join("")}</div>` : ""}
      <div class="presentation-executive-copy"><strong>${escapeHtml(artifact.overallAssessment || "Current assessment")}</strong><p>${escapeHtml(artifact.executiveSummary || message.text || "")}</p></div>
    </section>

    <div class="presentation-dashboard-grid presentation-dashboard-top">
      <section class="presentation-panel priorities-panel"><div class="presentation-panel-heading"><div><span class="intel-panel-kicker">Strategy</span><h2>Strategic priorities</h2></div></div>${renderPresentationIntelList(priorities)}</section>
      <section class="presentation-panel themes-panel"><div class="presentation-panel-heading"><div><span class="intel-panel-kicker">Deck intelligence</span><h2>Key themes</h2></div></div>${renderPresentationIntelList(themes)}</section>
      <section class="presentation-panel contacts-panel"><div class="presentation-panel-heading"><div><span class="intel-panel-kicker">People & contributors</span><h2>Who to contact</h2></div><span class="presentation-panel-count">${contributors.length || 0}</span></div>${renderContributorCards(contributors)}</section>
    </div>

    <div class="presentation-dashboard-grid presentation-dashboard-bottom">
      <section class="presentation-panel"><div class="presentation-panel-heading"><div><span class="intel-panel-kicker">Decision intelligence</span><h2>Decisions & asks</h2></div></div>${renderPresentationIntelList(decisions)}</section>
      <section class="presentation-panel"><div class="presentation-panel-heading"><div><span class="intel-panel-kicker">Evidence in deck</span><h2>Notable metrics & claims</h2></div></div>${renderPresentationIntelList(metrics,{metric:true,detailKey:"context"})}</section>
      <section class="presentation-panel risk-panel"><div class="presentation-panel-heading"><div><span class="intel-panel-kicker">Attention</span><h2>Risks & gaps</h2></div></div>${renderPresentationIntelList(risks)}</section>
    </div>

    <div class="presentation-dashboard-grid presentation-dashboard-final">
      <section class="presentation-panel presentation-material-panel"><div class="presentation-panel-heading"><div><span class="intel-panel-kicker">Latest project material</span><h2>Presentation</h2></div></div>
        <div class="presentation-material-card">${thumbnail ? `<img src="${escapeHtml(thumbnail)}" alt="First slide preview of ${escapeHtml(presentation.name || "presentation")}" referrerpolicy="no-referrer"/>` : `<div class="presentation-material-fallback">${icon("folder", 28)}</div>`}<div><strong>${escapeHtml(presentation.name || "Google Slides presentation")}</strong><p>${escapeHtml([`${presentation.slideCount || stats.slidesAnalysed || ""} slides`, presentation.owner, presentation.location].filter(Boolean).join(" · "))}</p><div class="presentation-material-actions">${presentation.url ? `<button type="button" class="drive-action-primary" data-source-url="${escapeHtml(presentation.url)}">${icon("external", 14)} Open deck</button>` : ""}</div></div></div>
      </section>
      <section class="presentation-panel actions-panel"><div class="presentation-panel-heading"><div><span class="intel-panel-kicker">BrainT recommendation</span><h2>Recommended next actions</h2></div></div>${renderPresentationIntelList(actions,{detailKey:"detail"})}</section>
      <section class="presentation-panel evidence-note-panel"><div class="presentation-panel-heading"><div><span class="intel-panel-kicker">Evidence quality</span><h2>Contributor evidence</h2></div></div><p>${escapeHtml(artifact.evidenceNote || "Contacts are derived only from verified Workspace evidence available to BrainT.")}</p><div class="presentation-evidence-stats"><span>${Number(stats.slidesAnalysed || 0)} slides</span><span>${Number(stats.commentsReviewed || 0)} comments</span><span>${Number(stats.peopleSignals || 0)} people signals</span></div></section>
    </div>
    ${renderEvidencePanel(sources)}
  </section>`;
}
